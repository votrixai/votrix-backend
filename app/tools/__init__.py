...
from app.tools import cron, file_output, image, oauth, preview, search_tools, video

# name → full tool definition dict (used by provisioning to build agents.create() tools list)
TOOL_DEFINITIONS: dict[str, dict] = {
    d["name"]: d
    for module in (cron, file_output, image, oauth, preview, search_tools, video)
...
async def execute(name: str, input: dict, user_id: str, session_id: str | None = None) -> dict:
    ...
    if name in ("download_file", "publish_file"):
    ...
    if name == "search_tools":
        return await search_tools.handle(name, input, user_id)
