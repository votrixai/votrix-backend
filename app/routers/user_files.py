"""User files API — virtual filesystem CRUD for end user agents.

Routes:
  GET    /users/{user_id}/agents/{blueprint_agent_id}/files              — ls
  GET    /users/{user_id}/agents/{blueprint_agent_id}/files/read          — read file
  POST   /users/{user_id}/agents/{blueprint_agent_id}/files               — write/create file
  PATCH  /users/{user_id}/agents/{blueprint_agent_id}/files               — edit file
  DELETE /users/{user_id}/agents/{blueprint_agent_id}/files               — delete file
  POST   /users/{user_id}/agents/{blueprint_agent_id}/files/mkdir         — create directory
  POST   /users/{user_id}/agents/{blueprint_agent_id}/files/mv            — move/rename
  GET    /users/{user_id}/agents/{blueprint_agent_id}/files/grep          — regex search
  GET    /users/{user_id}/agents/{blueprint_agent_id}/files/glob          — glob match
  GET    /users/{user_id}/agents/{blueprint_agent_id}/files/tree          — full tree
"""
...
async def ls(
    ...
    return [file_list_entry_from_user(e) for e in entries]
...
async def write_file(
    ...
    return file_list_entry_from_user(row)
...
async def edit_file(
    ...
    file = await user_files.read_file(session, blueprint_agent_id, user_id, body.path)
    if not file:
...
async def delete_file(
    ...
    if recursive:
        ...
        file_exists = await user_files.exists(session, blueprint_agent_id, user_id, path)
        if not file_exists:
            raise HTTPException(status_code=404, detail=f"File not found: {path}")
        await user_files.rm(session, blueprint_agent_id, user_id, path)
...
async def move(
    ...
    await user_files.mv(session, blueprint_agent_id, user_id, body.old_path, body.new_path)
...
async def glob(
    ...
    return [file_list_entry_from_user(r) for r in results]
...
async def tree(
    ...
    return [tree_entry_from_user(e) for e in entries]
