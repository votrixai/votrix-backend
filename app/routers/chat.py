...
async def _log_event_bg(session_id: str, seq: int, event_type: str, event_body: str, event_title: Optional[str] = None) -> None:
    """Log a session event using a standalone session (safe for background tasks)."""
    async with session_scope() as s:
        await sessions_q.log_event(s, session_id, seq, event_type, event_body, event_title)
...
async def _stream_chat_ds(
    ...
    async def on_tool_event(event_type_name: str, event_title: str, body_json: str) -> None:
        ...
        ingest_tasks.append(asyncio.create_task(
            _log_event_bg(ctx.session_id, seq_counter[0], et, body_json, event_title)
        ))
        seq_counter[0] += 1
    ...
    async def run_turn() -> None:
        ...
        ingest_tasks.append(asyncio.create_task(
            _log_event_bg(ctx.session_id, 0, "user_message", user_text)
        ))
        ...
        ingest_tasks.append(asyncio.create_task(
            _log_event_bg(ctx.session_id, seq_counter[0], "ai_agent_message", agent_body)
        ))
...
async def chat_stream(
    ...
    await sessions_q.create_session(session, request.session_id, request.org_id, request.agent_id, request.channel_type or "web")
