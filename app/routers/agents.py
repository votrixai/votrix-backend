...
from app.db.queries import agents as agents_q, agent_files
...
async def create_agent(org_id: str, body: CreateAgentRequest):
    """Create a new agent. Optionally seed from an existing agent."""
    existing = await agents_q.get_agent(org_id, body.agent_id)
    if existing:
    ...
    # Seed from another agent if requested
    if body.seed_from:
        source = await agents_q.get_agent(org_id, body.seed_from)
        if not source:
            raise HTTPException(status_code=404, detail=f"Seed source agent '{body.seed_from}' not found")
        # Copy prompt sections from source (unless explicitly provided)
        for key, col in _PROMPT_COLS.items():
    ...
    row = await agents_q.create_agent(org_id, body.agent_id, **kwargs)

    # If seeding, copy files too
    if body.seed_from:
        source_files = await agent_files.tree(org_id, body.seed_from)
        for f in source_files:
            if f["type"] == "directory":
                await agent_files.mkdir(org_id, body.agent_id, f["path"])
            else:
                content_row = await agent_files.read_file(org_id, body.seed_from, f["path"])
                if content_row:
                    await agent_files.write_file(
                        org_id, body.agent_id, f["path"],
...
async def update_agent(org_id: str, agent_id: str, body: UpdateAgentRequest):
    ...
    if body.prompts:
        for key, col in _PROMPT_COLS.items():
            ...
            if val is not None:
                await agents_q.set_prompt_section(org_id, agent_id, key, val)

    if body.registry is not None:
        await agents_q.set_registry(org_id, agent_id, body.registry)
...
async def delete_agent(org_id: str, agent_id: str):
    """Delete an agent and all its files (cascade)."""
    existing = await agents_q.get_agent(org_id, agent_id)
    if not existing:
        raise HTTPException(status_code=404, detail="Agent not found")
    await agents_q.delete_agent(org_id, agent_id)


@router.get("/orgs/{org_id}/agents/{agent_id}/prompts/{section}")
async def get_prompt_section(org_id: str, agent_id: str, section: str):
...
async def update_prompt_section(org_id: str, agent_id: str, section: str, body: UpdatePromptSectionRequest):
    ...
    if not existing:
        raise HTTPException(status_code=404, detail="Agent not found")
    await agents_q.set_prompt_section(org_id, agent_id, section, body.content)
