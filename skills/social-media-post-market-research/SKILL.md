...
**执行步骤：**
1. 从 `mnt/memory/social-media-manager/marketing-context.md` 拿到竞争对手名单
...
**输出结论示例：**
> 为你的「产品推广」主题整理了一组 hashtag（共 12 个），覆盖大中小三个层级，预计单篇触达比纯用大标签提升约 40%。已更新到 `mnt/memory/social-media-manager/marketing-context.md`。
...
每类调研完成后，只更新 `mnt/memory/social-media-manager/marketing-context.md` 的 `## 市场调研` 对应 section，其他部分保持不变，格式如下：
