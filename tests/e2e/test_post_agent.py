...
Usage:
    python tests/e2e/test_post_agent.py
    python tests/e2e/test_post_agent.py --force    # re-upload skills
    python tests/e2e/test_post_agent.py --message "..." # single message and exit
    python tests/e2e/test_post_agent.py --force --attach /path/to/file.png  # attach file to first message
...
Append a message to start chatting:
    echo "帮我完成 setup，店名叫晨光咖啡" >> tests/e2e/test_input.txt
"""
...
async def run_provision(force: bool = False) -> tuple[str, str]:
    ...
    config = json.loads(
        (Path(__file__).parents[2] / "agents" / AGENT_SLUG / "config.json").read_text()
    )
    env_id = config["envId"]
